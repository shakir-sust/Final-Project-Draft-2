...
# Loading libraries 
library(tidyverse)    # dplyr, purrr, tidyr, readr, stringr, etc.
library(lubridate)    # dates → yday, month
library(janitor)      # clean_names()
library(daymetr)      # download_daymet()
library(tidymodels)   # recipe(), bake(), fit(), predict()
# (assumes final_spec and weather_prep already exist in your environment)
...
fe_test <- test_full %>%
  ...
  # select and rename to match training
  transmute(
    year, site, hybrid,
    month_abb,
    soil.ph       = soilp_h,    # adjust these names if your clean_names() differ
    soil.om.pct   = om_pct,
    soil.k.ppm    = soilk_ppm,
    soil.p.ppm    = soilp_ppm,
    dayl.s        = dayl_s,
